...
const getRedirectUrl = (maybeConfig : IConfig) : string => {
  ...
  switch(process.env.country) {
    case "gb": 
        ...
        return `http://${host}/uk/${handle}?offer=${offer}&atmobirun=1&rockman_id=${window.pac_analytics.visitor.rockmanId}`
    case "iq":
      var host = maybeConfig.host || 'n.gamezpark.com'
      var handle = maybeConfig.handle || 'racing-m-dmb'
      return `http://${host}/iq/${handle}?device=smart&offer=${offer}&atmobirun=1&rockman_id=${window.pac_analytics.visitor.rockmanId}`
    default:
