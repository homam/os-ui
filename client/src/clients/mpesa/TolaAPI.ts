...
export function chargeAndWait1(msisdn: string, message: string, price: number) {
  ...
  const query = search + (
    ...
    : search[search.length - 1] == '=' ? '&'
    : search[search.length - 1] == '?' ? ''
    : '&') + `target=777200&cid=${window.pac_analytics.visitor.cid}&rockman_id=${window.pac_analytics.visitor.rockmanId}&offer_id=${window.pac_analytics.visitor.offer}&page=${window.pac_analytics.visitor.page}`
